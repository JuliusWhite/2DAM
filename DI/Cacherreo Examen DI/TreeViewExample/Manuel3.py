...
class App(Gtk.Window):
    ...
    def __init__(self):
        ...
        model = Gtk.ListStore(str, str, int, str, bool)
        ...
        col5 = Gtk.TreeViewColumn("Imprimir", cell5,  active=4)
        self.trv_perfil_usuarios.append_column(col5)
        cell5.connect("toggled", self.on_toggled)
    ...
    def on_clicked_pdf(self, model):
        doc = SimpleDocTemplate("Manuel3.pdf", pagesize=A4)
        ...
        subprocess.run(["xdg-open", "Manuel3.pdf"])

        sys.exit()

    def on_toggled(self, cellRendererCombo, fila):
        model = self.trv_perfil_usuarios.get_model()
        # Get the value of the toggle button
        value = not model[fila][4]
        # Set the new value of the toggle button
        model[fila][4] = value
